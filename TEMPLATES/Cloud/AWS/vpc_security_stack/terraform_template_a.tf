...
resource "aws_instance" "az_a_asav" {
...
resource "aws_instance" "az_a_ftd" {
...
/*
  ENI az_a_asav
*/
resource "aws_network_interface" "az_a_asav_e1" {
	subnet_id = "${aws_subnet.az_a_outside_f5_asav.id}"
	private_ips = ["${var.az_a_asav_e1}"]
	security_groups = ["${aws_security_group.SG_All_Traffic.id}"]
	source_dest_check = false
	attachment {
		instance = "${aws_instance.az_a_asav.id}"
		device_index = 1
	}
}

resource "aws_network_interface" "az_a_asav_e2" {
	subnet_id = "${aws_subnet.az_a_asav_ftd.id}"
	private_ips = ["${var.az_a_asav_e2}"]
	security_groups = ["${aws_security_group.SG_All_Traffic.id}"]
	source_dest_check = false
	attachment {
		instance = "${aws_instance.az_a_asav.id}"
...
/*
  ENI az_a_ftd
*/
resource "aws_network_interface" "az_a_ftd_e1" {
	subnet_id = "${aws_subnet.az_a_management.id}"
	private_ips = ["${var.az_a_ftd_e1}"]
	security_groups = ["${aws_security_group.SG_All_Traffic.id}"]
	source_dest_check = true
	attachment {
		instance = "${aws_instance.az_a_ftd.id}"
		device_index = 1
	}
}

resource "aws_network_interface" "az_a_ftd_e2" {
	subnet_id = "${aws_subnet.az_a_asav_ftd.id}"
	private_ips = ["${var.az_a_ftd_e2}"]
	security_groups = ["${aws_security_group.SG_All_Traffic.id}"]
	source_dest_check = false
	attachment {
		instance = "${aws_instance.az_a_ftd.id}"
...
resource "aws_network_interface" "az_a_ftd_e3" {
	subnet_id = "${aws_subnet.az_a_inside_f5_ftd.id}"
	private_ips = ["${var.az_a_ftd_e3}"]
	security_groups = ["${aws_security_group.SG_All_Traffic.id}"]
	source_dest_check = false
	attachment {
		instance = "${aws_instance.az_a_ftd.id}"
